# FFA / OEV Project State — Archive

Closed, self-contained sagas moved out of `docs/ai-project-state.md` to keep the active file readable. These are fully resolved and not referenced by any current gate or active track. If a task genuinely needs this history (e.g. Clip Extractor regresses again), read it here first.
