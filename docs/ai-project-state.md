...
**Last reconciled:** 5 July 2026 (MOG2-primary gate check: first attempt failed on stuck offer, exposed a termination-output gap in `360-mog2-detector.yml` — fixed (`42b2a68`), account confirmed clean, redispatched as run `28743015271`, DISPATCHED/UNVERIFIED; see Ball tracker — MOG2-primary track section)
...
**Gate check — run `28742355266` FAILED (5 July 2026):** offer `40871014` (AMD EPYC 7K62) got stuck in `loading` for the full 4-minute retry window and never reached `running` — no code bug, matches prior transient host-flakiness pattern. **Termination gap found:** the Launch step's own inline cleanup on this failure path used a single endpoint (`cloud.vast.ai/api/v1`) wrapped in bare `except: pass`, and never populated the `instance_id` step output — so the workflow's separate proven 3-endpoint termination step (`if: always()`) saw an empty instance ID and no-op'd ("No instance ID — nothing to terminate"). **Correction:** the account showing 0 instances afterward was because Johnson manually terminated it himself, not because the inline cleanup worked — this was a real leak, not a near-miss. **Root-caused further:** `playcam-poc.yml` already has the correct pattern (multi-offer retry with a proper `delete_instance()` 3-endpoint helper used at every cleanup site, `instance_id` only ever emitted once an instance is confirmed selected, `::error::` annotation on unrecoverable termination failure) — `360-mog2-detector.yml` was a partial reimplementation instead of a copy of that proven block. **Fixed properly (`0a08c68`):** replaced `360-mog2-detector.yml`'s offer-search + launch steps with the same proven multi-offer-retry/`delete_instance()` pattern from `playcam-poc.yml` (adapted to its CPU-only offer query), and upgraded its final termination step's failure case to `::error::` for visibility. **Standing rule added to `CLAUDE.md` (`d1bb04d`):** any future vast.ai-element workflow must reuse this proven lifecycle block verbatim rather than reimplementing it; only the offer query is script-specific. Redispatched as run `28743015271` (on the pre-fix version, already in flight when the fuller fix landed) — DISPATCHED, UNVERIFIED. **Not yet done:** the same old pattern likely still exists in other vast.ai workflows (e.g. `360-stage1-candidates.yml`) — flagged, not fixed, out of scope for this ticket; apply the new CLAUDE.md rule next time any of them are touched.
