...
**Fix (proposed, pending dispatch): use the `-runtime-` CUDA image variant, not `-devel-`.** `nvidia/cuda:12.4.1-runtime-ubuntu24.04` — includes CUDA runtime libraries (`libcudart` etc., fixing `cudaGetDevice`) without the full toolkit/compiler bulk of the `-devel-` variant that caused the original ~9GB cold-pull stall. Ubuntu 24.04 base is retained either way (needed for FFmpeg 6.1 apt packages, confirmed unaffected by the runtime/devel choice — both variants are built FROM the same Ubuntu 24.04 base with normal apt repos). No CUDA toolkit/`nvcc` is needed at build time (`cargo build` doesn't invoke it), only the runtime libs at execution time for `cudaGetDevice`.

**Next:** swap the image field to `nvidia/cuda:12.4.1-runtime-ubuntu24.04`, verify diff, merge, dispatch. Confirm (a) launch reaches `running` without the cold-pull stall (runtime variant is meaningfully smaller than devel), (b) `reco stitch` gets past `cudaGetDevice` this time, (c) `panorama.mp4` is produced and uploaded to the `Stitched/` Drive folder. If green, run the second pair (`GX010198`+`GX010175`) and Johnson's visual sign-off on the better of the two is the M1 gate.
