# FFA / OEV — Current Status (updated 24 Aug 2026)
...
- **Workflow cleanup + runner-contention fix — TO DO, not started (24 Aug).** Two asks from Johnson: (1) fix intermittent long gaps in the `youtube-relay.yml` schedule (observed ~1hr gap despite `*/5 * * * *` cron, runner online/idle at the time — cause not confirmed, no Vultr access this session); (2) remove unused automation — explicitly XbotGo (`xbotgo-scanner.yml`, `xbotgo-concat.yml`) and Vast.ai-backed workflows. Full audit done this session: 120 workflows registered total. Only 4 run on the actual `self-hosted` Vultr runner on a recurring schedule (YouTube Relay 5min, Sheet Sync 30min, Sync YouTube Cookies to R2 4h, Clip Extractor 6h) — Vast.ai/XbotGo don't run there, so removing them won't fix the relay gap on its own. ~65 of the 120 are stale registrations pointing at deleted workflow files (404 on `main`, still listed "active") — mostly the EXP7/EXP8 and OEV one-shot/temporary block — safe to disable regardless, zero functional risk. 13 workflows are Vast.ai-tagged or Vast.ai-GPU-backed (9 explicitly named, 4 OEV ones not). `Deploy Labeling Tool`/`Deploy Labeling Tunnel`/`Open Labeling Tool Firewall`/`Check Labeling Drive Sync` show **active** despite being reported disabled back on 6 Aug — needs re-checking before touching. **Open question before acting:** does "don't use vast anymore" cover the whole 360 Ball Tracker / OEV Vast.ai-GPU pipeline, or just the standalone Vast.ai-labeled ones — several non-Vast-named "360" workflows may still dispatch to Vast.ai internally, would need opening each to confirm.

**Runner-separation scope, narrowed (24 Aug):** ffa-media should run on its own dedicated runner, separate from the 360/OEV pipeline it currently shares a box with. Scope is just the 2 workflows Johnson actually wants moved: `youtube-relay.yml` and `sync-yt-cookies.yml`. `sheet-sync.yml` is being deprecated soon, not moving. `clip-extractor.yml` not mentioned — stays put unless said otherwise. Two implementation options discussed, not chosen yet: (a) second runner process/label on the same Vultr box — cheap, no new infra, but still shares CPU/disk/network with OEV renders; (b) a real second small Vultr VM — full isolation, avoids a repeat of the 20 Aug disk-full incident, costs ~$5-6/mo. Deferred at Johnson's request — revisit next session.
...
- **YouTube cookies -> R2 for ffa-media worker — MERGED, deployed, real probe not yet confirmed.** `sync-yt-cookies.yml` merged to main (`0e4b6993`); first dispatch (`32354185782`) ran clean end to end including the R2 upload — `secrets/yt_cookies.txt` confirmed present in R2 (7,031 bytes). The 4 R2 secrets (`S3_ENDPOINT_URL`/`S3_ACCESS_KEY_ID`/`S3_SECRET_ACCESS_KEY`/`S3_BUCKET`) were added to this repo this session, copied from ffa-media's Railway `worker` service vars. `ffa-media` consumer side (`feature/worker-r2-cookie-sync`) merged to its main (`6c12e615`) and deployed SUCCESS on both `web`/`worker` (Railway `83531b4f`/`3d4fec95`). **Not yet verified:** no real YouTube probe job observed succeeding post-deploy — check `ffa-media`'s `worker` logs on Railway after the next real footage link. See `ffa-media/docs/ai-project-state.md` for the consumer-side detail.
...
- **OEV hard-case 134–139s — B2a REJECTED, B2b visually inconclusive, downstream camera-response damping now prime suspect (5–6 Sep 2026).** Full arc: B2a (coast-time ball motion prediction in `ball.rs`) mechanically confirmed active but visually **rejected by Johnson — "still shit, does the same thing on the video."** Root-caused: the 134-139s window is ~98% covered by the existing production backward-bridge (`Bridged` state), leaving almost no live-`Coasting` frames for B2a to affect. B2b (allow `Tracking`+`Bridged` — not `Coasting`/`Lost` — to hold B1 local-action authority in `field.rs`'s `b1_local_centroid`/`b1_aim_target`) is code-verified working exactly as designed (`b1_active=true`, `local_centroid=true` on every sampled frame across the full window) but Johnson's visual check: **"not much better."** Suspicion, now code-supported: the *director* (target decision) is correct; the *executor* (`field.rs`'s velocity-clamped chase) is filtering it out downstream via a **0.20 rad (~11.5°) dead-zone**, a 0.18 rad/s velocity ceiling, and a 0.06 velocity-EMA — all introduced/tuned in the 1 Jun 2026 commit `bb1d38a964` ("calmer lookahead defaults, wider dead-zone, 2x reactivity"), confirmed unchanged since. Additionally, `run_loop.rs`'s `centered_smooth()` (a large centered moving-average over `post_smooth_half = lookahead_frames/2` past+future poses) runs **after** the panner and is what `PanDecision`/`pan_decision.pose` telemetry actually reflects — confirmed via code read, not the raw pre-smooth panner output. **Important prior-evidence caveat:** the proposed candidate values (`dead_zone_rad`→0.06, `max_velocity_rad_per_sec`→~0.31, `velocity_alpha`→~0.08) are nearly identical to the already-tested "Responsive panner tuning run" (`31997785854`, see Ball-state bridging section below) — that run still had the tracked ball outside the horizontal frustum on ~17.71% of known-ball frames, including multi-second misses while the tracker was actively `Tracking` — so camera-response tuning alone did not fully solve containment previously. A B2b + camera-response combined A/B is the untested combination. Branches in play: `video-stitcher@diag/hard-case-134-139s` (B1 diagnostic base), `video-stitcher@feat/b2a-coast-motion-prediction` (rejected, kept for reference), `ffa-automations@experiment/actionstate-b2b-bridged-authority-v2` (B2b, code-verified, visually inconclusive).

- **Spare-ball full-`Lost` reacquisition -- decaying grace window MERGED, CI-verified; test infra built and mostly verified, real-footage confirmation still blocked on one small bug (7 Sep 2026).** Separate sub-thread from the hard-case entry above: velocity-aware mid-coast reacquisition (`d525ed2`) fixed catch-up during coasting but Johnson found the spare-ball pull still happens right after a full `Lost` (~25s mark), root-caused to zero positional memory on fresh acquisition post-loss. Fix (`video-stitcher@experiment/decaying-reacquire-grace-01`, merged into `experiment/velocity-aware-ball-reacquire-01` at `28b6d820c6`): decaying soft trajectory preference for 750ms post-loss, confidence-only after. CI-verified (`cargo test`: 77/77 incl. all 4 new tests, both debug+release) -- one real `cargo fmt` defect caught and fixed first. **Next action, not yet done:** debug-logged confirmation run on real sample footage (same shape as run `34088607555`) to check it actually reduces the visual spare-ball pull -- do not wire into any `EXPECTED_RECO_SHA` production gate before that. Test infra for this (isolated pinned-SHA experimental binary build, known-bad-host skip -- verified firing live, two Cloudflare `User-Agent` fixes) is built on branch `experiment/ffa-automations@test/spare-ball-experiment-binary-override`, none of it upstreamed to `main` yet. The `cargo: command not found` bug was fixed (`bb25b926`) and a real confirmation run completed on `EU-RO-1` (`sample_02`, 180s, yolo26m, run `34131580642`, artifact SHA-verified experimental binary `28b6d820c6`, acceptance PASS). **Visual review (Johnson, 7 Sep):** footage "generally worse" -- two issues found in the event telemetry, NEITHER caused by the reacquisition fix itself:
1. A genuine ~13-frame (~0.2s) full ball loss (`world.ball=None`, not even bridged) at 13.2s, plus 5 more None-windows later in the clip (up to 71 frames / ~1.2s at 173s) -- raw dual-camera detector gaps, pre-existing, outside what the reacquire-grace fix touches (it only engages after a loss ends).
2. Sluggish camera response -- this run used `--panner-preset broadcast` with no override, i.e. the same dead_zone=0.20rad/max_velocity=0.18rad-s/velocity_alpha=0.06 tuning from `bb1d38a964` already identified as the prime suspect in the hard-case 134-139s investigation above. Same root cause, different clip.
**Next action, not yet done:** re-run with the B2b + camera-response candidate tuning (dead_zone->0.06, max_velocity->~0.31, velocity_alpha->~0.08) to see if it also improves this clip, OR treat panner tuning as a separate ticket from the spare-ball merge decision. See full entry below (search "Confirmation-run dispatch attempts") for detail.

**NEW (7 Sep, at Johnson's request): reconstructed pre-regression "v4" baseline + ROI fix, to see how close it gets on its own.** Johnson was happy with run `31913398625` (the accepted quality reference: YOLO26m, FOV 44, lookahead 1.5, cluster_alpha=0.08, "v3 hysteresis + v4 micro-damping"). That behaviour was never committed to video-stitcher -- it was a chain of uncommitted, SHA-pinned source patches applied at build time (ffa-automations commits c91314ea=V3, 8febf7a9=V4 delta, both since removed from main), on top of Reco `f27cbb6d0d65fcf9a11fb4d82d119ae214695318`. Reconstructed that exact patch chain (verified byte-identical against those commits) and stacked the ball-ROI `+0.40` vertical-margin relaxation (from `916529a61f`) on top, on a new branch `test/v4-hysteresis-roi-relax-01` (based off `test/spare-ball-experiment-binary-override`). Deliberately EXCLUDES all newer work (frame-stride, B2b, decaying-reacquire-grace, dead_zone/velocity-clamp retune) -- point is to see how close the pre-regression behaviour + the now-understood ROI fix gets to a working product on its own, before deciding what to layer back in.

Two real bugs found and fixed while assembling this (both because the ROI patch was originally written against a newer commit than `f27cbb6d`): (1) `with_ball_roi_diagnostics(..., frame_stride)` referenced a `frame_stride` var that doesn't exist at this base commit (predates the frame-stride feature) -- hardcoded to `1`; verified locally that `reco-autocam`/`reco-core` compile clean at this SHA before redispatching. (2) stitch args unconditionally passed `--frame-stride 1`, which isn't a valid CLI flag at this commit either -- removed; confirmed via CLI arg listing that every other flag used (`--panner-config`, `--panner-preset`, `--lookahead`, `--detection-interval`, etc.) does exist at `f27cbb6d`.

Redispatch succeeded on `EUR-IS-1` (run `34156511918`, acceptance PASS, real AI tracking + GPU zero-copy confirmed). **Visual review (Johnson, 7 Sep):** tracking was "alright" overall, but **the camera repeatedly fixates on a stray ball sitting in the goal.** Telemetry confirms and quantifies this: `world.ball` returns to the *exact same fixed coordinate* (yaw~0.967, pitch~0.005-0.01) in 20 separate lock-on episodes across the 180s clip, totaling ~45s+ of screen time (longest single lock: 6.3s at 106s) -- far too repeatable to be the real moving match ball; almost certainly the stray ball Johnson saw in the goal.

**Root cause, not a new bug:** this build deliberately has NONE of the later mitigation work (no reacquire-grace, no B2b bridging) -- there is nothing in this reconstruction that distinguishes "the real match ball" from "a stray ball sitting in the goal." The v3/v4 trajectory-hysteresis patch (`stabilize_world_ball`) is actually a *contributing* factor: once locked onto a high-confidence static detection, it requires 18 continuous frames of a competing trajectory within 4deg to override, making it sticky on the wrong target by design (the same mechanism that makes it resist noise). The ROI `+0.40` vertical-margin relaxation may also be what let this stray ball through in the first place -- goals sit near the field-polygon boundary, exactly where the tighter pre-relaxation ROI used to clip detections.

**Conclusion: this is the SAME spare-ball problem the reacquire-grace work was built to solve, just visible in its rawest form because this build has zero defenses against it.** The ROI fix alone does not get to a working product -- confirms some form of "don't fixate on a static ball" logic (reacquire-grace, or a stray-ball exclusion zone/mask around the goal specifically) is still required on top of the ROI fix and the v3/v4 camera dynamics. **Next action, not yet done:** decide whether to (a) layer reacquire-grace back on top of this v4+ROI-relax base, (b) add a static/stray-ball exclusion mask around the goal area specifically, or (c) some combination -- then redispatch on sample_02 again to check.
...
**Decaying post-loss reacquisition grace window -- DESIGNED, BUILT, CI-VERIFIED, MERGED (7 Sep 2026).** Built by ChatGPT (routed via GitHub connector, not Codex) against the exact spec above, branch `experiment/decaying-reacquire-grace-01` off `d525ed206740336973d1f46fcb4dbb2d1bc76857`, single file `crates/reco-autocam/src/trackers/ball.rs`. Implementation: on full `Lost`, `last`/`last_velocity` are now retained (not nulled) alongside a `lost_at_ms` timestamp; a new `lost_reacquire_score()` blends `confidence*(1-w) + trajectory_proximity*w` with `w` decaying linearly from `DEFAULT_REACQUIRE_INITIAL_WEIGHT=0.40` to 0 over `DEFAULT_REACQUIRE_GRACE_MS=750.0` (both overridable via new builder methods, explicitly flagged as tuning defaults not fixed constants); `trajectory_proximity` is a normalized `[0,1]` term using `max_jump_rad` purely as a normalization radius, never a rejection gate. `expire_lost_memory()` runs at the top of every `update()` call and clears the retained state once the window elapses -- note this makes the `elapsed_ms >= grace` early-return inside `lost_reacquire_score()` itself dead code in practice (expiry always fires first on the same frame), so the boundary-expiry behavior is actually delivered via the pre-existing fresh-acquisition `score()` path once `was_lost` goes false, not via that branch. Confirmed by manual trace, not treated as a defect -- end behavior is correct either way, just worth knowing if this code is touched again.

**Verification (no local Rust toolchain used this time -- CI-based instead):** first push failed `cargo fmt --check` on one `assert!` line (real defect, correctly caught, not merged). Claude fixed the one-line formatting issue directly via the GitHub contents API (single-file edit, no clone) after being told to. Re-verified by opening a diagnostic-only PR (#8, closed immediately after, never intended for merge) against `main` purely to force `rust.yml` CI to run (branch pushes to `experiment/*` don't trigger it). Final result on head `5d51a74909c0d79e75275ef0c79de5099c6c89e5`: `Test` job green, `reco-autocam` suite **77 passed, 0 failed**, all 4 new tests confirmed by name in the log (`full_loss_retains_trajectory_memory_without_reemitting_lost`, `lost_grace_yields_to_much_higher_confidence_mid_window`, `lost_grace_expires_to_pure_confidence_at_boundary`, `lost_grace_does_not_block_far_new_possession`), ran clean in both debug and release passes. `Check & Lint` also failed on this commit but for unrelated pre-existing reasons (clippy debt in `reco-core/src/session/vram_pool.rs`, not our file); `Check (Windows)`/`Check (Windows ARM64)` failed on an unrelated pre-existing `wgpu-hal`/D3D12 Windows toolchain issue; `cla`/`cargo-deny`/`Security Audit` are policy/dependency jobs, not code-correctness signal. None of these touch `ball.rs`.

**Merged:** `28b6d820c66ed42b2d2d6a3caa2fa657e417c852` -- `experiment/decaying-reacquire-grace-01` merged into `experiment/velocity-aware-ball-reacquire-01` (parent branch, still off frozen `d525ed2`). **Not yet done:** a real-footage debug-logged confirmation run (same shape as run `34088607555` above) to check whether this actually reduces/eliminates the ~25s spare-ball pull Johnson flagged -- CI proves the logic is correct in isolation, not that it fixes the visual symptom. Do not wire into `EXPECTED_RECO_SHA` on any production-facing workflow until that visual confirmation happens.


## 2026-09-07 (cont'd): Confirmation-run dispatch attempts, real-footage test still not achieved, one important discrepancy found

**Discrepancy found and corrected:** the "Fixed" `User-Agent` commit for `oev-runpod-sample-baseline.yml` documented above (`8eeca9d`) was **never actually merged into `main`** -- confirmed via `git compare`, it's orphaned 15 commits off main's history, not on any branch tip. `main`'s live pod-create `headers` dict had no `User-Agent` key when checked directly. So every claim above that this was "Fixed" was inaccurate for this specific workflow's `main`. Cherry-picked it (verbatim, same diff) onto a new test branch this session -- **still not on `main`.**

**Test branch created:** `test/spare-ball-experiment-binary-override` (off `main` `c0d397f1b9`). Holds test-only infrastructure, none of it upstreamed to `main` yet:
- The cherry-picked `User-Agent` fix (create-pod headers) -- commit `fccca428`.
- Same `User-Agent` fix applied to the *separate* Terminate-step headers dict (a second, distinct dict in the same file, same gap) -- commit `7c7d1779`.
- **Known-bad-host client-side skip logic** added to `oev-runpod-sample-baseline.yml`'s pod-allocation loop (commit `c143e568`, RunPod's API has no "exclude machine" param -- confirmed against their official OpenAPI schema -- so this is done in the loop instead): tracks `seen_machine_ids` across the run; a repeat `machine_id` is deleted and retried **without consuming a real attempt**; independent `MAX_TOTAL_POD_CREATES=8` hard ceiling added as a safety backstop. **Verified firing correctly live** in run `34117831164`: got the same `machine_id=hnf9nqmhzlsf` twice in a row, correctly skipped the second without counting it, no wasted preflight.
- **Separate pinned-SHA experimental reco-cli binary build** added to `runpod_sample_baseline_yolo26_remote.sh` (non-frozen; `runpod_bootstrap.sh` itself untouched, still only builds the gated production binary at `main`'s pinned SHA to satisfy `EXPECTED_RECO_SHA`). Clones/checks out `video-stitcher` at the exact pinned experimental SHA (`--detach`, hard-fails on any drift from the target SHA), builds it into a separate `CARGO_TARGET_DIR`, logs both the source SHA and the built binary's `sha256`, then swaps `RECO_BIN` to that binary for the actual test run -- lets us test unmerged code without touching `video-stitcher@main`.

**Bugs introduced and found this session (both in the experimental-binary build block above, not in `ball.rs`):**
- `time` bash keyword didn't resolve in this remote shell context (`command not found`, exit 127) -- fixed, dropped it (diagnostic-only, not load-bearing).
- **Still unfixed:** `cargo: command not found` (exit 127) -- the experimental build subshell doesn't inherit whatever profile/`~/.cargo/env` sourcing makes `cargo` available to the frozen bootstrap's own SSH session (separate SSH invocation, doesn't inherit that PATH setup). Needs `source ~/.cargo/env` (or equivalent) added inside the experimental build subshell. **This is the next concrete fix.**

**7 dispatches today** against `oev-runpod-sample-baseline.yml` on the test branch, datacenters `EU-RO-1` and `EUR-IS-1` (confirmed both have the sample pack staged and reachable -- `GX010197-seed1384188843` verified present via Google Drive fallback path, downloaded successfully in one run; false alarm earlier in session about it being "missing" was a misread of printed script source, not an actual failure). Outcomes: 3x pure RunPod capacity ("no instances currently available", $0 spent, confirmed not a publicized RunPod outage -- status page showed 100% uptime), 1x genuine NVDEC preflight fail on a real new host (correctly handled), 1x known-bad-host skip fired correctly (see above), 2x blocked by the `cargo` PATH bug above. **Real cost today: under $0.20 total, no orphaned/billable pods** (all confirmed terminated, either in-loop or via the fixed Terminate-step headers).

**Still not achieved:** an actual real-footage run of the merged `experiment/velocity-aware-ball-reacquire-01` grace-window fix. All infrastructure for testing it safely (without touching `main`) is now built and mostly verified except the one `cargo` PATH bug above.

**Next session, in order:**
1. Fix the `cargo` PATH issue in the experimental build block (`source ~/.cargo/env` or equivalent) on `test/spare-ball-experiment-binary-override`.
2. Redispatch `oev-runpod-sample-baseline.yml` against that branch (try `EU-RO-1` and `EUR-IS-1`, capacity has been intermittent both places today).
3. Once a run actually completes: check whether the ~25s spare-ball pull is visually reduced, per the original goal.
4. Decide whether the known-bad-host skip logic and the two `User-Agent` fixes get upstreamed to `main` properly (they're currently only on the disposable test branch) -- they're generically useful regardless of the ball-tracker experiment's outcome.
