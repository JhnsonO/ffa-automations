...
**Next:** inspect run `31279245133` — confirm (a) launch step reaches `running` well inside the poll window this time, (b) `nvidia-smi`/`vulkaninfo` succeed in `env.log`, (c) `cargo build` completes past the `reco-io`/`Pixel::VAAPI` stage, (d) `reco calibrate` + `reco stitch` complete against the `GX010197`/`GX010173` pair. Inspect calibrate's match quality before treating stitch output as gate-worthy. If green, run the second pair (`GX010198`+`GX010175`) and Johnson's visual sign-off on the better of the two is the M1 gate.
