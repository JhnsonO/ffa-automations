...
**Last reconciled:** 5 July 2026 (MOG2-primary gate check VERIFIED — inconclusive/negative, no sustained lock at the real t≈60s goal; see Ball tracker — MOG2-primary track section for full evidence and open next-gate decision)
...
**Gate check — VERIFIED, INCONCLUSIVE-TO-NEGATIVE (5 July 2026):** run `28743015271` succeeded; pulled `mog2_candidates.json` (frames 1000–2300 of `equirect_full.mp4`) and converted pixel x → yaw (`(x/4032-0.5)×360`). **No sustained MOG2 lock at t≈60s itself** — zero detections in the expected off-frame-left region during t=60.0–60.9s (only unrelated blobs at yaw +20°/0°/−25°). The one long-sustained candidate in the window (yaw −40.4°, rock-steady for ~40 frames / 1.3s at t=57.06–58.36s) is almost certainly a static object, not the ball — too stable, and not far enough left to be "clipped at the frame edge" as Johnson described the real miss. Brief scattered single-frame blips near yaw −57°..−59° appear at t=61.9–62.6s (right zone, not sustained). The most coherent moving candidate (yaw −53.5°→−48.9°, ~0.8s smooth motion) doesn't appear until t=64.2–65.0s, several seconds after the adjudicated goal. **Caveat:** the −55°/−57° target yaw tested against was inherited from the retracted 3B.8 finding (wrong timestamp, t≈18.5s) — the real t≈60s goal is only pinned down as "beyond the ~±42.5° FOV edge, left side," not to an exact yaw. **Conclusion:** this does not support "MOG2 held the ball through the goal" — the honest read is inconclusive-leaning-negative. Committing to the MOG2-primary/cluster-fallback camera direction on this evidence alone would be premature. **Next gate (open, needs Johnson's steer):** either (a) visually inspect extracted frames at t≈59–65s to eyeball what's actually there at the candidate yaws found, or (b) treat this as sufficient signal to shelve/redesign the MOG2-primary proposal, or (c) accept the uncertainty and proceed anyway as a lower-confidence bet.
