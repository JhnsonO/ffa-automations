...
**Next action:** once run `31523236281` completes, if the CUDA EP check now passes, confirm `calibrate.log`/`stitch.log` run, confirm `field_roi` still injected/validated, confirm follow-cam acceptance passes, pull `timing.log` for stage breakdown, and Johnson visually watches `followcam.mp4` against the product question: does the follow-cam acquire the ball and produce good camerawork on a genuinely unseen segment. Do not tune panner/tracker/detector/ROI/speed until that visual review happens. If the CUDA EP check fails again with a *different* real error, that's meaningful new information — keep it, don't discard the response. If it fails with the *same* `libcublasLt.so.13` error, the pin didn't take effect and needs a fresh look (check pip resolved version in `env.log`).
