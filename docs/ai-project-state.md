...
**Next:** inspect run `31282877306` — confirm (a) launch reaches `running` fast (should, same image as the successful `ubuntu:24.04`-only run), (b) the new CUDA-runtime apt-install block succeeds in `env.log` (watch for the `cuda-runtime-12-4` package resolving — package name is version-specific and unverified against this exact apt snapshot), (c) `reco stitch` gets past `cudaGetDevice` this time, (d) `panorama.mp4` is produced and uploaded to the `Stitched/` Drive folder. If green, run the second pair (`GX010198`+`GX010175`) and Johnson's visual sign-off on the better of the two is the M1 gate. If the apt install itself fails (e.g. package name mismatch for this Ubuntu 24.04 snapshot), that's the next thing to fix — check `apt-cache search cuda-runtime` output in `env.log` for the actual available package name/version on this image.
