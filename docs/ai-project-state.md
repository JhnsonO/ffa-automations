...
**Next:** apply the `env` dict change, verify diff (single file, single hunk expected), merge, dispatch against the `GX010197`/`GX010173` pair. Confirm (a) `reco stitch` finally gets past `cudaGetDevice`, (b) `panorama.mp4` is produced and uploaded to `Stitched/`. Once that happens: (1) build-caching (deferred item, now finally in scope), (2) run the second pair (`GX010198`+`GX010175`), (3) Johnson's visual sign-off on the better of the two is the M1 gate.
