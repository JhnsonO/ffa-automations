...
**Last reconciled:** 6 July 2026 (`playcam/fusion_target_yaw.py` corrected, `209227b` supersedes `db6205a` — ball-lock source moved from raw MOG2 candidates to the established tracker's own Stage 2 anchor tracklets, per Johnson's explicit correction; validated offline against the real 120s clip; not judged render-ready yet — see Ball tracker — MOG2-primary track section)
...
### Offline 4-tier target_yaw fusion — CORRECTED (6 July 2026, `209227b`, supersedes `db6205a`)

**Correction applied this session:** the first version of `playcam/fusion_target_yaw.py` (`db6205a`) built its own tier-1 ball-lock directly from raw MOG2 candidates (confidence + geometry + a custom continuity gate). Johnson corrected this: the established ball tracker (Stage 1/1b/2, frozen per Frozen tracker boundaries below) is the established ball source and must not be bypassed, re-diagnosed, or second-guessed by a new script's own heuristics. That version's 70%-of-samples tier-1 fire rate only showed the prototype was promoting raw blobs too freely, not that the real tracker needed tightening. **`db6205a`'s approach is retired, not extended.**

**Rebuilt (`209227b`):** tier-1 now reads directly from the established tracker's own Stage 2 output (`tracklets.json`, artifact `7944978610` from run `28355256427` — the same evidence already documented under Ball tracker — MOG2-primary track), using only tracklets with `status=="anchor"` — Stage 2's own strictest, pre-existing category (`stage2_temporal_link.py`: `MIN_OBS_FOR_ANCHOR=8`, `MIN_MEAN_CONF_FOR_ANCHOR=0.20`, `MIN_COVERAGE_FOR_ANCHOR=0.50`, `MIN_ANCHOR_STRENGTH=0.55` — reused verbatim, not modified). No interpolation within a tracklet's span; only directly-observed frames are used, so tier-1 coverage is honestly sparse rather than manufactured. No additional continuity/persistence gate is applied on top — Stage 2's own linker already established these as coherent, and re-gating them would repeat the same mistake. `ball_tracker/mog2_blob_filter.py` is unused by this script now (left in place, not deleted — still valid for other diagnostic purposes) and no tracker file was modified, re-run, or diagnosed.

**Behavioural changes matching the corrected design intent:** (1) tier-1 target_yaw = anchor yaw + a small bounded ahead-of-play lead (`TIER1_LEAD_OFFSET_DEG=3deg`) in the sustained flow direction, when flow is sustained — previously tier-1 ignored flow entirely. (2) tier-2 (flow-hold) and tier-3 (flow-only bias) logic is otherwise unchanged in structure (same constants: `HOLD_MAX_SAMPLES=4`, `HOLD_FLOW_NUDGE_DEG=2deg`, `FLOW_BIAS_NUDGE_DEG=4deg`). (3) added a final circular-safe EMA smoothing pass over the whole target_yaw sequence (`SMOOTHING_ALPHA=0.35`, reusing the same alpha precedent as a prior activity-bias signal) to damp step changes at tier transitions, per Johnson's "with smoothing" spec. Output CSV now includes both raw `target_yaw` and `target_yaw_smoothed` for inspectability.

**Verified against real data** (same clip, zero paid compute; tracklets.json frame range 6-3596 matches the mog2/play_location frame range 6-3596/0-3595 from the two prior signals — treated as the same ~2-minute source clip, not independently re-verified via exact metadata match). Tier distribution across 240 samples: ball-lock=66 (27.5%), flow-hold=31 (12.9%), flow-only=53 (22.1%), centroid=90 (37.5%) — far more plausible than the retired version's 70%, and consistent with the tracker's known sparse anchor coverage (25 anchors total across 3596 frames). Flow-only now genuinely gets used (53 samples vs 1 in the retired version).

**Key windows (as requested):**
- **t~57-58s (site of the old, now-irrelevant raw-MOG2 false lock):** no anchor tracklet has an observed frame anywhere in t=54.5-59.5s. This is not a bug to fix — it is Stage 2's own honest evidence that this stretch has no tracker-confirmed ball position. Flow is not sustained here either (quiet, matching the earlier player_flow_bias.py finding that fewer players were moving fast enough right at the shot). Fusion falls to tier-4 centroid, landing near -1 to -5deg (near-center), both raw and smoothed.
- **t~59-60s (the missed goal):** same gap — no anchor coverage, flow not sustained, centroid fallback near -1 to -2.6deg. The fusion does not show a strong lean toward the actual goal side (~-55deg) at the moment of the shot itself, because neither validated source (tracker or flow) has anything to say there in this specific clip.
- **Did it preserve the t=41-54s leftward flow lean?** Yes, more clearly than the retired version. t=40-43s: tier-1/tier-2 anchor-based values already sit left (-21 to -26deg) with flow agreeing. t=48-54s: tier-3 (flow-only) runs almost continuously, nudging centroid consistently left (raw -1 to -10deg, smoothed -2 to -5deg) — this is the ahead-of-play signal expressing itself as intended, no longer crowded out by an over-permissive tier-1.
- **Did it avoid wrong-direction signals?** Yes. The one large raw swing in this window (t=43.544, centroid-fallback jumps to +49.69deg — a genuine independent centroid reading, not a fusion defect) is visibly damped by the smoothing pass (smoothed value 3.39deg vs raw 49.69deg), which is the smoothing doing real, visible work rather than being cosmetic.

**Honest limitations, unchanged or new:** (1) even `status=="anchor"` is only ~80% human-precision (20/25 adjudicated as likely-ball) — tier-1 still carries the tracker's own residual false-positive rate, not something this script can improve on. (2) The tracker has genuinely no confirmed evidence through the entire goal window in this clip — a real, structural gap, not something fusion logic can paper over; only a render/visual check or a differently-instrumented clip could tell us whether the pre-goal flow lean would have been enough on its own. (3) The tracklets.json/play_location.jsonl clip-alignment is inferred from matching frame ranges, not independently confirmed. (4) Still one clip, still no render, still no paid compute (issue #8 one-clip risk unchanged).

**Blocker:** none for what was built and validated offline. **Next gate:** Johnson's read on whether this corrected offline timeline — precise-but-sparse anchor positions, a genuine pre-goal ahead-of-play lean that fades before the shot, and honest centroid fallback where the tracker has nothing — looks sensible enough to justify a first real render test, or whether the sparse tier-1 coverage on this particular clip means a second, better-covered clip should be tried offline first.
