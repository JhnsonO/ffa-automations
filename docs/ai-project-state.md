...
**Last reconciled:** 5 July 2026 (MOG2-primary gate check dispatched — run `28742355266`, DISPATCHED/UNVERIFIED, testing real t≈60s goal directly on `equirect_full.mp4`; see Ball tracker — MOG2-primary track section)
...
**Gate check — DISPATCHED, UNVERIFIED (5 July 2026):** existing `ball_tracker` MOG2 artifacts (`7942126312`/`7944978610`) turned out to be built from `equirect_trim.mp4` (Drive `1z2p2FgLsjgvIIBw0HZXWEenckMLWpVNX`, 2.07GB, ~120s) — a separate, smaller Drive file from `equirect_full.mp4` (8.06GB, 482.48s, the actual missed-goal clip), and Johnson does not recall/know the trim's offset into the full recording, so those artifacts could not be used to check the real t≈60s goal. Johnson's call: skip the offset reconciliation, test directly. Dispatched the standalone `360-mog2-detector.yml` (CPU-only, no Stage 0/YOLO prereq) directly against `equirect_full.mp4`, frames 1000–2300 (native ~29.97fps → t≈33–77s, bracketing the t≈60s goal with warm-up margin either side), `mog2_var_threshold=16`, `mog2_history=500` (matches validated baseline). Run `28742355266` — DISPATCHED, UNVERIFIED. Output artifact will be `mog2-candidates-28742355266` (`mog2_candidates.json`, per-frame top-5 blob candidates). Next: pull the artifact, convert frame index → yaw (equirect width, `yaw=(x/width-0.5)*360`), and check for continuous confident single-blob presence near the known goal-mouth yaw (≈−55°..−57°, left side) through the t≈55–65s frames before committing to the MOG2-primary direction.
