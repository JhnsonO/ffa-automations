...
**STOPPING HERE — debug budget exhausted for this problem.** This session has now tried 6 distinct fix attempts against the same underlying GPU-access failure (4 base-image swaps, 1 CUDA package fix, 1 env-var fix) without resolving it, all guesses based on plausible-but-unverified theories about Vast.ai's launch API rather than confirmed documentation. Per protocol, stopping to avoid a 7th blind guess.

**Risk / open question, unresolved:** it's not confirmed that Vast.ai's launch API `env` field actually maps to Docker container environment variables the way assumed — it may control something else entirely (e.g. port-forwarding config), which would explain why setting `NVIDIA_VISIBLE_DEVICES` had zero effect. This needs to be checked against Vast.ai's actual API documentation before further code changes, not guessed again. Everything else this pipeline needs is confirmed working: launch (when SSH doesn't flake), build (VAAPI wall resolved), CUDA runtime package install (dynamic resolution working), and `reco calibrate` (100% confidence, repeatable). The single remaining blocker is real GPU device access not reaching the container for `reco stitch`'s CUDA-based video source init.

**Next (for a fresh chat, or after doc verification):** (1) check Vast.ai's actual API docs for what `env` in the `/asks/{offer_id}/` PUT request controls — search "vast.ai api asks env docker environment variables" or check their official API reference/GitHub if linked from docs. (2) If `env` isn't the right mechanism, look for the correct one — could be a different request field, or Vast.ai's `--gpus`/GPU passthrough might need an explicit `runtype` variant, or a Vast.ai-specific onstart script hook. (3) Once GPU access is confirmed reaching the container (verify via a quick `nvidia-smi`-inside-cargo-run + a real Vulkan/CUDA device showing in the `reco_core::gpu` log line, not `llvmpipe`), rerun stitch against `GX010197`/`GX010173`. (4) Once `panorama.mp4` is produced: build-caching (deferred item), second pair (`GX010198`+`GX010175`), Johnson's visual sign-off = M1 gate.
