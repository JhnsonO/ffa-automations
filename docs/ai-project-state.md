...
**Next gate:** run `28700595596` DISPATCHED — UNVERIFIED (full 482.48s `equirect_full.mp4`, `271b701`). This is the first run expected to actually cover the whole clip; on completion check whether it now contains a genuine second stoppage/restart and re-attempt the follow→wide→follow validation report.
