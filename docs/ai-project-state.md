...
**Verification run `31305380382`: DISPATCHED — UNVERIFIED.** https://github.com/JhnsonO/ffa-automations/actions/runs/31305380382 — head commit `acd4fce`. Inputs: `left_clip=trimmed_GX010197.MP4`, `right_clip=trimmed_GX010173.MP4`.

**Minor unrelated note:** `gh.sh dispatch` returned HTTP 400 on this run (quoting issue with the `inputs` JSON argument through its own curl call) — worked fine when the same payload was sent via a direct curl POST to the dispatches endpoint. Not investigated further this session; gh.sh's `dispatch` subcommand may need a quoting fix if this recurs.

**Next:** pull `env.log` from this run first, regardless of whether stitch passes or fails — it now contains the ground-truth diagnostic dump (NVIDIA env vars actually seen by the container, `/dev/nvidia*` presence, ICD manifest state, and whichever `libnvidia-gl-*`/`libnvidia-decode-*` version got installed or why not). If `reco_core::gpu` in `stitch.log` shows a real NVIDIA device (not `llvmpipe`) and `panorama.mp4` is produced, the fix worked — proceed to build-caching (deferred item), second pair (`GX010198`+`GX010175`), then Johnson's visual sign-off = M1 gate. If it still falls back to `llvmpipe`, the diagnostic dump should show which specific piece is still missing (env var not propagated vs. no matching driver package found vs. something else) rather than requiring another blind guess.
