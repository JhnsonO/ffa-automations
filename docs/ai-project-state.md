...
**Status: blocked on RunPod capacity, not code.** Both pinned datacenters exhausted: `EU-RO-1` (1 attempt) and `EUR-IS-1` (3 attempts) all returned `HTTP 500 "no instances currently available"` before any pod launched -- $0 spent across all of them. The code itself is believed correct (build succeeded on the one run that got a pod; only the CLI-flag issue surfaced after that, now fixed and not yet run). **Next action, not yet done:** redispatch `oev-runpod-sample-baseline.yml` on branch `test/v4-hysteresis-roi-relax-01`, sample_02/180s/yolo26m/lookahead=1.5/cluster_alpha_override=0.08, either datacenter, whenever capacity frees up.
