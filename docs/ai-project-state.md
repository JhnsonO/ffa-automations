...
Run `28700595596` CANCELLED (predated the `dcbff31` GPU/CPU-floor fix — snapshotted at `271b701` only, would still have risked a Blackwell card). `vastai-instance-check.yml` dispatched to confirm no orphan from the cancel. **Next gate:** run `28700918611` DISPATCHED — UNVERIFIED, head `c51987d` (has both `271b701` max-frames fix and `dcbff31` GPU allowlist/cpu_cores=16) — first run with all current fixes. On completion: confirm GPU selected is not Blackwell, check whether the full clip now contains a genuine second stoppage/restart, and re-attempt the follow→wide→follow validation report.
