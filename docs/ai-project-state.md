...
Subsystem: `.github/workflows/gopro360-upload.yml` + `gopro360/vastai_stitch.sh` (MAX1, target 1.72x) + `gopro360/vastai_stitch_max2.sh` (MAX2 8K, target ~0.90x realtime — recalibrated 2026-07-01, see below). Distinct from the tracker; shares the repo only.
...
- **Pool expansion (2026-07-01, commit `db8b97aab5`):** `cpu_cores` floor 32→16, `cpu_ghz >= 4.5` filter removed, GOOD_CPU_SUBSTRINGS expanded with `Threadripper PRO`, `Threadripper`, `Core Ultra 9`.
- **MAX2 floor recalibration (2026-07-01, session 20b — SUPERSEDES prior MAX2 floor values above):** Pool expansion alone did not fix it — every offer, including a 96GB RTX PRO 6000 (24-core), failed the old 1.25x preflight floor at real 8K res (measured ~0.56–0.63x on best available hardware). The 1.25x/1.15x/2.87x MAX2 targets were never achievable at 8K; they were carried over from MAX1-era assumptions. Recalibrated against real measured ceiling (~0.5–0.9x), confirmed live in both `vastai_stitch_max2.sh` and `vastai_stitch_max2_chapter.sh`: `PREFLIGHT_MIN` 0.85→0.55, `MIN_SPEED` 1.3→0.55, `ABORT_MIN` 1.15→0.45, `TARGET_SPEED` 2.87→0.90.
- **Instance leak fix (2026-07-01):** offer-search loop cleanup used a single-endpoint, swallowed-exception termination call — leaked several billed instances this session. Replaced with a robust 3-endpoint logged termination function matching the proven final `terminate` step.
- **Parallel chapter pipeline — NEW SUBSYSTEM, VERIFIED WORKING (2026-07-01, session 20b):** GoPro's API exposes raw per-chapter source files independently of the server-side `concat` merge (`label="source"`, `.../source/default/{item_number}.360`) — no custom split step needed. Built to cut wall-clock time by stitching chapters in parallel instead of one long serial encode.
  - `gopro360/vastai_stitch_max2_chapter.sh` — stitches one chapter, uploads to Drive `FFA360/Inbox/` as `{session_prefix}_chNN.mp4`. Copied from production `vastai_stitch_max2.sh`; shares offer-search/redispatch logic and reputation system.
  - `.github/workflows/gopro360-chapter-upload.yml` — dispatches one chapter job, parallel-safe (own concurrency group per chapter).
  - `scripts/max2_chapter_concat.py` + `.github/workflows/max2-chapter-concat.yml` — verifies all expected chapters are actually present (real check), stream-copy concats by `item_number` order, **re-injects 360 XMP spherical metadata via exiftool** (concat's new container drops per-chapter XMP — would silently break YouTube 360° playback if missed), uploads, logs to `uploaded.db`, cleans up Inbox.
  - Required a stream-specifier fix (`[0:0]`/`[0:1]` → `[0:v:0]`/`[0:v:1]`) after `goprobe.yml` (new ffprobe diagnostic workflow) showed raw chapter files interleave audio/data between video streams differently than the concat variant.
  - **Verified end-to-end:** session `0419` (`GS010419.360`/`GS020419.360`/`GS030419.360`, media_id `6a42be4d8c9f5c76014416b7`) — chapter runs `28533197998` and `28533416803` succeeded (3rd chapter ~74s, too short to warrant its own job), concat+upload run `28551023163` completed **success**. Confirmed directly via GitHub Actions API 2026-07-02.
- **Also new this session:** `.github/workflows/vastai-instance-check.yml`, `vastai-orphan-cleanup.yml` (hourly cron — noted unreliable on low-traffic repos, do not rely on it alone for leak cleanup), `goprobe.yml` (ffprobe diagnostic).
- **Open:** `offers_exhausted` 36% baseline not yet re-measured against pool expansion + floor recalibration together — needs a few more runs to confirm. Reputation store is last-write-wins (acceptable at current volume).
