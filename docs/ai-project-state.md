...
**Last reconciled:** 6 July 2026 (`playcam/fusion_target_yaw.py` redesigned again, `47132ed` supersedes `209227b`/`db6205a` — unified anchor+decaying-flow-offset model with a hard venue clamp and a real rate limiter, per Johnson's safety spec; self-caught and fixed a wrong venue-range constant during smoke-testing before reporting; validated offline; not judged render-ready yet — see Ball tracker — MOG2-primary track section)
...
**Blocker:** none for what was built and validated offline. **Next gate:** Johnson's read on whether this timeline — a bounded, fading ahead-of-play lean on top of sparse-but-precise anchors and an honest centroid fallback where evidence is genuinely absent — looks sensible enough to justify a first real render test.
