...
4. Zero-copy NV12 corruption bug — handed off as its own separate ticket already (see the dedicated handoff prompt delivered this session); Phase 1 (free source read) already done and found a refined, code-grounded hypothesis (CUDA shared-memory allocation sized from a guessed pitch *before* Vulkan's real row-pitch/memory-requirement is known, in `create_shared_texture()`, `crates/reco-core/src/interop/vulkan.rs` — no check that `mem_reqs.size` fits the CUDA allocation). Not yet fixed.
