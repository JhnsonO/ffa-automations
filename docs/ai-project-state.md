...
**Blocker:** fix direction is a product/architecture decision, not a coding action. Candidate directions (not designed, not costed): (a) leading-edge framing — frame the cluster's extreme player in direction of travel instead of centroid; (b) goal-end bias — when centroid/leading players cross a yaw threshold toward a pitch end, bias target_yaw toward that goal mouth and/or widen FOV; (c) ball-position nudge — ball tracker frozen at 11.34% confirmed_ball_pct, too weak to lead but possibly usable as a bounded high-confidence nudge; (d) fix Action Zone candidate eligibility (3B.8 finding: main cluster can never be selected) so attack motion can steer the camera. **Next gate:** Johnson to confirm the exact goal timestamp (±2s) and which end, then arbitrate fix direction. No code changed this session.
