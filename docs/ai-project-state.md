...
**Last reconciled:** 5 July 2026 (v3 rejected by Johnson as worse than v2, reverted; v2 remains current working fusion version; direction decision made — next lever is MOG2 detection quality via geometric/velocity heuristic filtering on raw blobs, not more fusion-logic tuning; no code written yet, this is the next chat's build step; see Ball tracker — MOG2-primary track section)
...
**Not yet started:** no code written for the size/aspect-ratio/velocity heuristic filter yet — this is the next actual build step for a new chat.
