...
**Last reconciled:** 5 July 2026 (Johnson called for a real 2-minute MOG2-primary/cluster-fallback render rather than one-instant checks; found the existing `--yaw-source-csv` mechanism means no new architecture is needed; MOG2 data for t=0–120s done, Phase-1 pass in flight; see Ball tracker — MOG2-primary track section)
...
**In flight:**
- `360-mog2-detector.yml` run `28745136405` — **SUCCESS**. `equirect_full.mp4` frames 0–3596 (t=0–120s), baseline params. Artifact pulled: 3596 frames, 2033 with ≥1 candidate.
- `playcam-poc.yml` run `28746051636` — Phase-1-only pass (`skip_render=true`), same clip/window, to get real `play_location.jsonl` for t=0–120s. First dispatch attempt failed with HTTP 422 from a wrong input name guess (`start_offset_s`/`window_duration_s` instead of the real `start_seconds`/`duration_seconds`) — no run was created, corrected and redispatched. DISPATCHED, UNVERIFIED.

**Next (blocked on the Phase-1 pass above):** once `play_location.jsonl` lands, build the fusion script locally, write `mog2_fusion_comparison.csv`, commit it, then dispatch `playcam-poc.yml` for real with `action_zone_csv_path` pointing at it and `skip_render=false` to get the actual 2-minute render.
