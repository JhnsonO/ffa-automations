...
**Gate check — run `28742355266` FAILED (5 July 2026):** offer `40871014` (AMD EPYC 7K62) got stuck in `loading` for the full 4-minute retry window and never reached `running` — no code bug, matches prior transient host-flakiness pattern. **Termination gap found:** the Launch step's own inline cleanup on this failure path used a single endpoint (`cloud.vast.ai/api/v1`) wrapped in bare `except: pass`, and never populated the `instance_id` step output — so the workflow's separate proven 3-endpoint termination step (`if: always()`) saw an empty instance ID and no-op'd ("No instance ID — nothing to terminate"). Checked the account directly via `vast-cleanup.yml`: **0 instances, confirmed clean** — no cost leak this time, but the proven step was never actually exercised; got lucky. **Fixed (`42b2a68`):** `360-mog2-detector.yml` now writes `instance_id` to `$GITHUB_OUTPUT` immediately after creation, not only on the success path, so the proven termination step always has it regardless of how Launch exits. Two-line diff, nothing else touched. **Redispatched** as run `28743015271` (same params; this workflow has no offer-exclusion input so the bad offer couldn't be explicitly skipped, but offer selection re-queries the live market each dispatch) — DISPATCHED, UNVERIFIED. **Not yet done:** the identical single-endpoint-inline-cleanup pattern likely exists in sibling vast.ai workflows (at least `360-stage1-candidates.yml` shares the same boilerplate) — flagged, not fixed, out of scope for this ticket.
