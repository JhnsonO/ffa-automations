...
**Session note:** this session has run well past the normal 3-debug-cycle budget (credit-topup reset the immediate blocker but the underlying script has needed several real, distinct fixes since: disk sizing, launch disk param, error-body visibility, CUDA-runtime cache poisoning, onnxruntime version pin). Each fix has been a genuine new finding with log evidence, not a repeat guess, which is why this kept going rather than handing off — but if `31524916652` fails, the right move is a fresh-chat handoff regardless, rather than continuing to extend this session further.

**Next action:** once run `31524916652` completes, if the CUDA EP check now passes, confirm `calibrate.log`/`stitch.log` run, confirm `field_roi` still injected/validated, confirm follow-cam acceptance passes, pull `timing.log` for stage breakdown, and Johnson visually watches `followcam.mp4` against the product question: does the follow-cam acquire the ball and produce good camerawork on a genuinely unseen segment. Do not tune panner/tracker/detector/ROI/speed until that visual review happens.
