...
**Debug budget reached this chat (3 cycles: streaming fix → FFmpeg dev packages → VAAPI version wall) — stopping per protocol, handing off.**

**Next (for a fresh chat):** two paths to unblock, neither started:
1. Try a newer/different Vast.ai base image with a more current FFmpeg (5.x/6.x) that matches what reco-project's CI likely uses — check reco-project's own CI config (`.github/workflows` in their repo) for the exact FFmpeg version/image they build against, then match it here.
2. Check if `reco-io`'s `ffmpeg` Cargo feature can be built without the VAAPI hardware-encode path (e.g. a feature flag to disable hwaccel, since we may not need VAAPI specifically — `wgpu` handles the actual stitch rendering, VAAPI here is likely just for encode output). Worth checking `reco-io/Cargo.toml` feature flags before assuming a full image swap is needed.

Once `reco-cli` builds successfully, the pending next step is unchanged: run `reco calibrate` + `reco stitch` on the two Aylestone trimmed pairs (`GX010197`+`GX010173`, `GX010198`+`GX010175`), inspect calibrate's match quality before committing to stitch, then Johnson's visual sign-off is the M1 gate.
