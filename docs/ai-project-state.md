...
**Next gate — tougher validation window (reliability pass only):** run `28699134976` CANCELLED (wrong source — Johnson wanted the converted MAX2 chapter, not the old session file). Replaced by run `28699224749` DISPATCHED — UNVERIFIED (4 July 2026, `995a44d`): entire converted chapter `0421_ch01.mp4`, Drive ID `10D_Zhntym2rO00aISMsMYU7m5CRtRURU` (11.5 GB, 7680×3840 MAX2, 18.2 min), start=0, duration=1200 (caps at file end). `vastai-instance-check.yml` dispatched to confirm no orphan from the cancelled run. Purpose: prove real follow → wide → follow on footage containing stoppages/restarts/spread play, not just wide → follow. On completion report only: all mode-transition timestamps; whether follow→wide→follow occurred; camera snap / yaw lag / FOV issues; ball loss during transitions; overall watchability. No targeting redesign, no threshold/FOV tuning (follow 85 / wide 100 held), no action-centre or ball-guidance, `ball_tracker/` untouched. Confirm instance terminated.
