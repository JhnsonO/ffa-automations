...
**Verification run `31283387937` redispatched as `31284XXX` (pending — see next run id in a future update): DISPATCHED — UNVERIFIED.** Same inputs (`left_clip=trimmed_GX010197.MP4`, `right_clip=trimmed_GX010173.MP4`), same head commit `1d9d629` (no code change, just a retry against a hopefully-more-cooperative host).

**Next:** inspect the redispatched run — confirm (a) SSH comes up this time (pure infra roll of the dice), (b) if it does, confirm the CUDA runtime package resolves in `env.log` (this is still the first real test of that fix), (c) `reco stitch` gets past `cudaGetDevice`, (d) `panorama.mp4` produced and uploaded to `Stitched/`. Once that happens: (1) build-caching per above, (2) run the second pair (`GX010198`+`GX010175`), (3) Johnson's visual sign-off on the better of the two is the M1 gate.
