...
**Last reconciled:** 6 July 2026 (`playcam/fusion_target_yaw.py` (`db6205a`) built and validated offline against the real 120s clip; caught and fixed a real implausible-jump bug in tier-1 via continuity gating; open finding — tier-1 fires on 70% of samples and likely crowds out the flow-only tier even in its strongest window (t=41-54s); not judged render-ready yet — see Ball tracker — MOG2-primary track section)
...
### Offline 4-tier target_yaw fusion (6 July 2026, `db6205a`)

**Built:** `playcam/fusion_target_yaw.py` — standalone, offline, no render, no paid compute, no renderer/tracker/frozen files touched. Imports and reuses `ball_tracker.mog2_blob_filter.filter_candidates()` and `playcam.player_flow_bias.compute_flow_signal()` directly rather than duplicating logic. Priority order per ~2Hz sample (play_location.jsonl's native cadence, not full frame rate): (1) ball-lock — confident (conf>=0.55), geometrically-clean, multi-frame-persisted MOG2 candidate; (2) flow-hold — ball-lock lost within the last 4 samples (~2s), hold near the last ball yaw, nudged by current flow direction if present; (3) flow-only bias — no recent ball-lock, but flow signal is *sustained*, nudge centroid_yaw gently (4deg) in that direction; (4) centroid fallback.

**Self-caught bug, fixed before reporting:** the first working version's tier-1 had no continuity check — it picked the single highest-confidence qualifying candidate independently in each ~0.5s window. Smoke-testing against the real clip showed target_yaw swinging ~80-90 degrees between consecutive samples (e.g. -50.2 to -58.0 to -15.4 to +8.1 across four samples), because MOG2's confidence+geometry filtering can't distinguish the ball from other clean, confidently-detected moving blobs (e.g. players' feet/legs), and a different one can win each independent window. Added a continuity gate (`MOG2_MAX_JUMP_DEG_PER_SEC=60`, `MOG2_MAX_REACQUIRE_GAP_SEC=2.0s`): a new tier-1 candidate must be within a plausible jump of the last accepted ball position, unless the ball's been missing longer than the reacquire gap. **This bound is a rough, conservative placeholder — not derived from real ball-speed/camera-geometry data — and is explicitly flagged as needing future validation, not a solved problem.**

**Verified against real data** (same clip/artifacts as the two prior signals, zero paid compute): after the continuity fix, tier distribution across 240 samples is ball-lock=168 (70%), flow-hold=68 (28%), flow-only=1 (0.4%), centroid=3 (1%). Per-sample target_yaw jumps are now bounded and no longer show implausible teleports.

**Key windows (as requested):**
- **t~57-58s (known false MOG2 lock at yaw~-40.4deg from the raw-candidate bug):** correctly never appears — the static-suspect flag from `mog2_blob_filter.py` excludes it as designed. Instead fusion holds (tier 2) at yaw~-50.6deg from t=57.057-57.558 (carried from a ball-lock accepted at t=56.556), then reacquires a new clean candidate at yaw~-12.5deg at t=58.058. **Honest caveat:** the ~-50.6deg value held through this window is itself unverified — it could be a genuine ball position or another quasi-static false lock that evaded the raw-frame static filter (which requires <5px drift over 15 *consecutive* frames) while still not being real ball motion. No ground truth available to confirm either way from this data alone.
- **t~59-60s (the missed goal, left side):** fusion sits at yaw~-29 to -30deg through t=59.059-60.06 (tier 1/1/2) — left of centre, directionally consistent with the known goal side, but roughly 25deg short of the actual goal-mouth angle (~-55deg per the 3B.8 investigation). It then swings to +21deg at t=60.56 (tier 1), consistent with a post-goal restart/clearance.
- **Did it preserve the t=41-54s leftward flow lean?** Only partially, and this is the single most important finding from this pass: because tier-1 (ball-lock) fires on 70% of all samples, it wins almost continuously through this exact window too, so tier-3 (flow-only bias) essentially never gets to act on the signal that was the strongest, most validated evidence from the player-flow session (it fired once in the whole 240-sample run). The underlying flow direction data is unchanged and still correct; the fusion as currently thresholded mostly overrides it rather than deferring to it.
- **Did it avoid wrong-direction signals?** Yes — no instantaneous large-angle flips remain after the continuity fix; the largest per-step change in the 52-62s window is ~50deg over a ~1s gap (within the time-scaled gate), not a teleport.

**Not yet done / unproven:** (1) whether tier-1's confidence+geometry+persistence+continuity combination genuinely tracks the ball rather than other plausible moving blobs — no ground truth exists to confirm this, and the ~70% fire rate is higher than what would be expected if it were ball-specific; (2) tier-1's dominance means the flow-only tier (the most validated signal from this session) is largely unused in this run; (3) the jump-rate/reacquire-gap constants are rough placeholders, not derived from real ball-speed data; (4) no render, no visual confirmation, no second clip tested (one-clip overfitting risk still open, issue #8).

**Blocker:** none for what was built and validated offline. **Next gate:** this offline timeline is NOT yet judged sensible enough on its own to justify a render — the open question above (tier-1 over-permissiveness crowding out the flow signal) needs a decision first: either tighten tier-1 further (longer persistence, or a way to reject candidates that coincide with a known player track position) so tier-3 can actually operate where flow was strongest, or deliberately test a reordering/weighting where flow gets more say even when a weak ball-lock exists. Johnson's call before any paid compute is spent.
