...
**Fix @ (pending), `oev_reco_stitch_remote.sh` only: stop hardcoding a CUDA package version — resolve it dynamically.** Try the unversioned `cuda-runtime` meta-package first (NVIDIA repos generally alias this to the current default release); if that 404s, fall back to `apt-cache search` for any `cuda-runtime-N-M` package actually present in the live repo, `sort -V` for the highest, and install that. Removes the need to guess/hardcode a version number that can go stale as NVIDIA rotates their published packages.

**Storage/caching question (Johnson, this session): explicitly deferred until a run succeeds.** Vast.ai's persistent storage attaches to one specific instance; this workflow deliberately shops across up to 5 different offers/hosts per run for resilience, so a storage volume wouldn't reliably carry over between runs unless we gave up that multi-offer retry and pinned to a single host. Better fit for the actual goal ("don't rebuild every time"): build the `reco` binary once, upload the compiled binary to the OEV Drive folder, and have the script check for/download a prebuilt binary before falling back to a full `cargo build` from source — portable across whichever host gets picked, no Vast.ai storage costs, cuts ~10 min build down to a download. **Do not implement this until a run has actually produced a working `reco` binary + successful stitch — no point caching a broken build.** Revisit as the very next thing once `panorama.mp4` is produced successfully.

**Next:** apply the dynamic-package-resolution fix to `oev_reco_stitch_remote.sh`, verify diff (single file), merge, dispatch against the `GX010197`/`GX010173` pair. Confirm (a) the CUDA runtime package actually resolves and installs this time (check `env.log`), (b) `reco stitch` gets past `cudaGetDevice`, (c) `panorama.mp4` is produced and uploaded to `Stitched/`. Once that happens: (1) build-caching per above, (2) run the second pair (`GX010198`+`GX010175`), (3) Johnson's visual sign-off on the better of the two is the M1 gate.
