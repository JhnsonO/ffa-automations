...
module.exports = async function handler(req, res) {
  ...
  try {
    if (q.raw) {
      ...
      const [rows] = await pipeline([['LRANGE', 'hc:raw', 0, n - 1]]);
    ...
    if (q.type) {
      ...
      const [val] = await pipeline([['GET', `hc:latest:${q.type}`]]);
      if (!val) return res.status(404).json({ error: 'no data for type' });
      return res.status(200).json(JSON.parse(val));
    }
    const [types] = await pipeline([['SMEMBERS', 'hc:types']]);
    const names = (types || []).sort();
    const vals = names.length ? await pipeline(names.map((t) => ['GET', `hc:latest:${t}`])) : [];
