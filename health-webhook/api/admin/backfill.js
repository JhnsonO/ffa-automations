...
// POST /api/admin/backfill?dry=1 -> counts only. Remove this file after the one-off run.
const { isAuthorised } = require('../../lib/auth');
const { pipeline, k } = require('../../lib/redis');
const { buildDayWrites, writeAggregates } = require('../../lib/aggregate');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const tally = { payloads: 0, records: 0, skipped: 0, hashes_touched: 0, days: {} };
    ...
    for (let end = total - 1; end >= 0; end -= 3) {
      ...
      for (const row of (rows || []).slice().reverse()) {
        const body = (JSON.parse(row) || {}).payload; if (!body) continue;
        tally.payloads++;
