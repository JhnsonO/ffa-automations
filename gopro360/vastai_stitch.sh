...
# instead of after a multi-GB download + partial encode. Hang-safe by
# construction: synthetic source, hard -t and -frames:v caps, timeout wrapper,
# progress to a file (no blocking pipe), and -f null (no disk write).
BENCH_W=4032
BENCH_H=2388
PREFLIGHT_MIN=1.55     # = MIN_SPEED (1.6) - 0.05; reject hosts that can't meet the sustained floor
...
MIN_SPEED=1.6          # sustained floor — below this on 3 consecutive windows → reject and redispatch
ABORT_MIN=1.45         # = MIN_SPEED - 0.15; instantaneous floor per sampling window
