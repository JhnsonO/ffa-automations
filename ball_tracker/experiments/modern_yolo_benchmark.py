...
YAW=(0,90,180,270); W,H,FOV=1280,720,110.0; REPO="keremberke/yolov8-football-ball-detection"
 ...
 for tid in BENCHMARK_TRACKLETS:
  ...
  for fr in frames(ts.get(tid,{})):
   ...
   for yaw in YAW:
    ...
    im=Image.fromarray(cv2.cvtColor(cv2.resize(crop,(640,360)),cv2.COLOR_BGR2RGB));dr=ImageDraw.Draw(im);best=max(ds,key=lambda z:z['football_conf'],default=None)
    if best:
     x1,y1,x2,y2=best['bbox_xyxy'];cx,cy=(x1+x2)/4,(y1+y2)/4;dr.ellipse((cx-8,cy-8,cx+8,cy+8),outline='lime',width=3);dr.text((8,8),f"{best['football_conf']:.2f}",fill='white')
    else:dr.text((8,8),'×',fill='red')
    dr.text((8,342),f'{tid} | frame {fr} | yaw {yaw}°',fill='white');panels.append(im)
   rows.append(panels)
  if rows:
   sh=Image.new('RGB',(2560,360*len(rows)),'black')
   for i,row in enumerate(rows):
    for j,im in enumerate(row):sh.paste(im,(640*j,360*i))
